...
(function () {
    ...
    function getBadgeWrapper() {
        ...
        elem.style.top = '85%'; // use 'top' instead of 'bottom' because moveBadge() needs the top position
        elem.style.right = '5%';
        // elem.style.top = '70vh';
    ...
    // todo: it's called "touch support" but not really tracking the touch events.
    function addTouchSupport(badge) {
        ...
        function onMouseUp(event) {
            ...
            // Remove thyself.
            event.currentTarget.removeEventListener(event.type, onMouseUp, true);
        }
    ...
    function onFirstVoicesChanged(event) {
        ...
        document.addEventListener("mouseup", speakSelectedText, true);
    ...
    function init() {
        ...
        function start(event) {
            if (!state.initialized) {
                ...
                window.addEventListener('offline', onFirstOffline, true);
                window.addEventListener('offline', onOffline, true);
                window.addEventListener('online', onFirstOnline, true);
                ...
                if (speechIsSupported()) {
                    ...
                    window.speechSynthesis.addEventListener('voiceschanged', onFirstVoicesChanged, true);
                    // prune and populate new voices as the system is updated...
                    window.speechSynthesis.addEventListener('voiceschanged', onVoicesChanged, true);
                }
                else {
