...
const preamblePath = path.join(__dirname, 'preamble.js');
// This path is relative to the baseUrl set later.
const amdLoaderPath = path.join(__dirname, 'node_modules/alameda/alameda');
...
optimizerPaths[moduleId.AMD_LOADER] = amdLoaderPath;
...
const onBuildRead = (moduleName, path, contents) => {
    ...
    return (moduleName === moduleId.AMD_LOADER) ?
        preamble + contents :
        contents;
};

const taskRunner = (grunt) => {
    // Task configuration.
    grunt.initConfig({
